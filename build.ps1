...
ForEach ($folder in $testDirs) { 
    ...
    exec { & dotnet test $folder.FullName -c Release --no-build --no-restore /p:CollectCoverage=true /p:CoverletOutput=$root\coverage /p:MergeWith=$root\coverage.json /p:Include="[*]CoreDemoApp.*" /p:Exclude="[*]CoreDemoApp.Testing.*" $format }
}
